...
def run_strategies(frames):
    ...
    for func, df_entry, df_secondary, cfg_idx in strategy_calls:
        ...
        move   = round(atr * rr, 2)
        sig = SignalResult(
            strategy=name, stype=stype, direction=direction,
            entry=round(entry,2), sl=sl, tp=tp, rr=rr,
            atr=round(atr,2), expected_move=move,
            reason=reason, filter_run=filter_run, pf=pf,
